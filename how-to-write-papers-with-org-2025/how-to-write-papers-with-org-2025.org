...
この記事は、[[https://adventar.org/calendars/11321][実空間コンピューティング研究室アドベントカレンダー]]4日目の記事です。
...
[fn:3]orgファイルからtexファイルへの変換はorg-modeによるものですが、組版のステップはTex環境およびlatexmkによるものです。
...
例えば情報処理学会の公式テンプレートを使用する場合、 ~\usepackage~ 以下のファイル冒頭は次のようになります。 ~#+begin_export latex~ と ~#+end_export~ で囲まれた箇所が、LaTeXでそのまま出力される[fn:5]ので、タイトルや著者情報が必ず所定の形式で出力されるようになります。
...
** 参考文献の参照
org-modeにデフォルトである、org-cite[fn:7]という機能を利用します。文中に ~[cite:@key]~ と書くことでLaTeXでいうところの ~\cite~ 形式の参照が利用できるようになるほか、 ~bib~ ファイルからの文献の検索と挿入が利用できるようになります。これにフロントエンドとしてcitar[fn:8]パッケージを組み合わせることで、モダンな絞り込み検索を可能にします
...
文献を選択するときにTAB、挿入するときにTABで、TABキーを2回押します。私はここでEnterを押し続け、参照を挿入することができなくて悩みました。
